...
import { twoLettersOneOrderApi } from '@/api/two-letters-one-order';
import type { LetterDocType, LetterDocument } from '@/types/two-letters-one-order';
import { downloadLetterDoc, formatLetterFileSize } from '@/utils/letter-download';
...
const tableData = ref<LetterDocument[]>([]);
const yearFilter = ref<number | undefined>();
const monthFilter = ref<number | undefined>();
const pagination = reactive({ current: 1, pageSize: 10 });

const columns: TableColumnType[] = [
  { title: '周期', dataIndex: 'periodLabel', key: 'periodLabel', width: 140 },
...
async function loadList() {
  loading.value = true;
  try {
    tableData.value = await twoLettersOneOrderApi.list({ type: props.type });
  } finally {
    loading.value = false;
  }
}
...
function handleDownload(doc: LetterDocument) {
  downloadLetterDoc(doc);
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'fileName'">
            <div class="file-cell">
              ...
              <span class="file-cell__name">{{ (record as LetterDocument).fileName }}</span>
            </div>
          </template>
          <template v-else-if="column.key === 'fileSize'">
            {{ formatLetterFileSize((record as LetterDocument).fileSize) }}
          </template>
          <template v-else-if="column.key === 'operation'">
            <a-button type="link" size="small" @click="handleDownload(record as LetterDocument)">
              <DownloadOutlined /> 下载
            </a-button>
          </template>
        </template>
