...
const needReason = computed(
  () => Boolean(props.record?.restore && props.record.restore.status !== 'pendingReview')
);
...
async function submit() {
  if (!props.record?.restore) return;
  ...
  try {
    await qingpuRestoreApi.revoke(props.record.restore.id, reason.value.trim() || undefined);
    message.success('恢复申请已撤销');
    emit('saved');
    close();
  } catch (e: any) {
...
<template>
  <a-modal
    ...
    @cancel="close"
  >
    <template v-if="record?.restore">
      <a-alert
        type="warning"
        show-icon
        class="alert"
        :message="`即将撤销恢复申请：${record.projectName}`"
        :description="`当前恢复状态：${QINGPU_RESTORE_STATUS_LABEL[record.restore.status]}`"
      />
