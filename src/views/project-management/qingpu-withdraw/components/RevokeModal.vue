...
async function submit() {
  ...
  try {
    await qingpuWithdrawApi.revoke(props.record.id, reason.value.trim() || undefined);
    message.success('退库申请已撤销');
    emit('saved');
    close();
  } catch (e: any) {
  ...
  >
    <template v-if="record">
      <a-alert
        ...
        :description="`当前退库状态：${QINGPU_WITHDRAW_STATUS_LABEL[record.status]}`"
      />
